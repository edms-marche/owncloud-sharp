	...
	public class Client {
		...
		public List<ResourceInfo> List(string path) {
			...
			foreach (var item in result) {
				...
				res.Name = HttpUtility.UrlDecode(item.Name);                                            // 2018.07.28 paulus: need to decode unicode file name
				res.QuotaAvailable = item.QutoaAvailable;
				res.QuotaUsed = item.QuotaUsed;
				res.Size = item.Size;
				res.Path = HttpUtility.UrlDecode(item.Uri.AbsolutePath.Replace("/" + davpath, ""));     // 2018.07.28 paulus: need to decode unicode file name
        ...
        /// 2018.08.13 paulus: add expireDate, refer: https://docs.nextcloud.com/server/13/developer_manual/core/ocs-share-api.html?highlight=sharetype
        /// </summary>
        ...
        /// <param name="expiredate">(optional) Set an expire date for public link shares. This argument expects a well formatted date string, e.g. ‘YYYY-MM-DD’.</param>
        public bool UpdateShare(int shareId, int perms = -1, string password = null, OcsBoolParam public_upload = OcsBoolParam.None, string expiredate = null) {
            ...
            if (expiredate != null)
                request.AddQueryParameter("expireDate", expiredate);
        ...
        {
            var request = new RestRequest(GetOcsPath(ocsServiceCloud, "users") + "/{userid}" + "/disable", Method.PUT);
            request.AddHeader("OCS-APIREQUEST", "true");
        ...
        {
            var request = new RestRequest(GetOcsPath(ocsServiceCloud, "users") + "/{userid}" + "/enable", Method.PUT);
            request.AddHeader("OCS-APIREQUEST", "true");
        ...
        /// <summary>
        /// 2018.08.02 paulus:
        /// Return a list of all user names (IDs)
        /// </summary>
        /// <returns></returns>
